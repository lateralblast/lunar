  ...
  if [ "${function}" = "eq" ]; then
    if [ "${actual_value}" = "${correct_value}" ]; then
      ...
      if [ ! "${set_name}" = "" ]; then
        case "${set_name}" in
          "--"*)
            verbose_message  "az backup vault update --name \"${workspace_name}\" --resource-group \"${resource_group}\" ${set_name} \"${correct_value}\"" "fix"
            ;;
          *)
            verbose_message  "az backup vault update --name \"${workspace_name}\" --resource-group \"${resource_group}\" --set \"${set_name}\"=\"${correct_value}\"" "fix"
            ;;
        esac
      fi
    fi
