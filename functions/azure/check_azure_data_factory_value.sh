...
check_azure_data_factory_value () {
  ...
  if [ "${function}" = "eq" ]; then
    if [ "${actual_value}" = "${correct_value}" ]; then
      increment_secure   "${description} for Data Factory \"${data_factory_name}\" with Resource Group \"${resource_group}\" Parameter \"${query_string}\" is \"${function}\" to \"${correct_value}\""
    else
      increment_insecure "${description} for Data Factory \"${data_factory_name}\" with Resource Group \"${resource_group}\" Parameter \"${query_string}\" is not \"${function}\" to \"${correct_value}\""
      if [ ! "${set_name}" = "" ]; then
        case "${set_name}" in
          "--"*)
    ...
    if [ "${actual_value}" = "${correct_value}" ]; then
      increment_insecure "${description} for Data Factory \"${data_factory_name}\" with Resource Group \"${resource_group}\" Parameter \"${query_string}\" is \"${function}\" to \"${correct_value}\""
      if [ ! "${set_name}" = "" ]; then
        case "${set_name}" in
          "--"*)
            verbose_message  "az datafactory update --name \"${data_factory_name}\" --resource-group \"${resource_group}\" ${set_name} \"${set_value}\""           "fix"
