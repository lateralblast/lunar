...
check_azure_redis_enterprise_cache_value () {
  ...
  if [ "${function}" = "eq" ]; then
    if [ "${actual_value}" = "${correct_value}" ]; then
      increment_secure   "${description} for Redis Enterprise Cache \"${redis_name}\" with Resource Group \"${resource_group}\" Parameter \"${query_string}\" is \"${function}\" to \"${correct_value}\""
    else
      increment_insecure "${description} for Redis Enterprise Cache \"${redis_name}\" with Resource Group \"${resource_group}\" Parameter \"${query_string}\" is not \"${function}\" to \"${correct_value}\""
      if [ ! "${set_name}" = "" ]; then
        case "${set_name}" in
          "--"*)
    ...
    if [ "${actual_value}" = "${correct_value}" ]; then
      increment_insecure "${description} for Redis Enterprise Cache \"${redis_name}\" with Resource Group \"${resource_group}\" Parameter \"${query_string}\" is \"${function}\" to \"${correct_value}\""
      if [ ! "${set_name}" = "" ]; then
        case "${set_name}" in
          "--"*)
            verbose_message  "az redisenterprise update --name \"${redis_name}\" --resource-group \"${resource_group}\" ${set_name} \"${correct_value}\""           "fix"
