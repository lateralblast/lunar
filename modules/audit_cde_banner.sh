...
audit_cde_banner () {
  if [ "$os_name" = "SunOS" ]; then
    ...
    for check_file in /usr/dt/config/*/Xresources ; do
      ...
      if [ -f "$new_file" ]; then
        check_file_value $new_file "Dtlogin*greeting.labelString" colon "Authorized uses only" star
        check_file_value $new_file "Dtlogin*greeting.persLabelString" colon "Authorized uses only" star
      fi
    done
  fi
}
