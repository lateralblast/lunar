  ...
  if [ "$os_name" = "SunOS" ]; then
    ...
    for cde_file in /usr/dt/config/*/Xresources ; do
      ...
      if [ -f "$check_file" ]; then
        check_file_value $check_file "Dtlogin*greeting.labelString" colon "Authorized uses only" star
        check_file_value $check_file "Dtlogin*greeting.persLabelString" colon "Authorized uses only" star
      fi
    done
  fi
}
