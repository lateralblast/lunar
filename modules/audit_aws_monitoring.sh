  ...
  if [ "$trails" ]; then
    ...
    for trail in $trails; do
      ...
      if [ ! "$metrics" ]; then
        ...
        funct_verbose_message "aws logs put-metric-filter --log-group-name $trail --filter-name network_gw_changes_metric --metric-transformations metricName=network_gw_changes_metric,metricNamespace='CISBenchmark',metricValue=1 --filter-pattern '{ ($.eventName = CreateCustomerGateway) || ($.eventName = DeleteCustomerGateway) || ($.eventName = AttachInternetGateway) || ($.eventName = CreateInternetGateway) || ($.eventName = DeleteInternetGateway) || ($.eventName = DetachInternetGateway) }'" fix
