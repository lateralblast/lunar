        ...
        if [ -f "$check_file" ]; then
          ...
          if [ "$check_value" != "$search_value" ]; then
            if [ "$audit_mode" = "1" ]; then
              ...
              verbose_message    "awk '( $1 == \"password\" && $2 == \"requisite\" && $3 == \"pam_cracklib.so\" ) { print $0  \" dcredit=-1 lcredit=-1 ocredit=-1 ucredit=-1 minlen=9\"; next }; { print }' < $temp_file > $check_file" "fix"
