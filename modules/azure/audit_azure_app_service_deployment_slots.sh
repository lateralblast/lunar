...
audit_azure_app_service_deployment_slots () {
  ...
  for app_name in ${app_names}; do
    ...
    for slot_name in ${slot_names}; do
      ...
      # 2.2.4   Ensure 'Basic Authentication Publishing Credentials' are 'Disabled' - TBD
      check_azure_app_service_deployment_slot_value "Basic Authentication Publishing Credentials" "${app_name}" "${slot_name}" "${res_group}" "basicPublishingCredentialsPolicies" "ftp" "Microsoft.Web/sites" "properties.allow"                  "eq" "false"                         ""                                      ""
      check_azure_app_service_deployment_slot_value "Basic Authentication Publishing Credentials" "${app_name}" "${slot_name}" "${res_group}" "basicPublishingCredentialsPolicies" "scm" "Microsoft.Web/sites" "properties.allow"                  "eq" "false"                         ""                                      ""
      # 2.2.5   Ensure 'FTP State' is set to 'FTPS only' or 'Disabled' - TBD
      check_azure_app_service_deployment_slot_value "FTP State"                                   "${app_name}" "${slot_name}" "${res_group}" "config"                             "ftp" "Microsoft.Web/sites" "ftpState"                          "eq" "${azure_ftp_state}"            "--ftp-state"                           ""
      # 2.2.6   Ensure 'HTTP version' is set to '2.0' (if in use) - TBD
      check_azure_app_service_deployment_slot_value "HTTP Version"                                "${app_name}" "${slot_name}" "${res_group}" "config"                             "web" "Microsoft.Web/sites" "http20Enabled"                     "eq" "true"                          "--http20-enabled"                      ""
      # 2.2.7   Ensure 'HTTPS Only' is set to 'On' - TBD
      check_azure_app_service_deployment_slot_value "HTTPS Only"                                  "${app_name}" "${slot_name}" "${res_group}" "config"                             "web" "Microsoft.Web/sites" "httpsOnly"                         "eq" "true"                          "httpsOnly"                             ""
      # 2.2.8   Ensure 'Minimum Inbound TLS Version' is set to '1.2' or higher - TBD
      check_azure_app_service_deployment_slot_value "Minimum Inbound TLS Version"                 "${app_name}" "${slot_name}" "${res_group}" "config"                             "web" "Microsoft.Web/sites" "minTlsVersion"                     "eq" "1.2"                           "--min-tls-version"                     ""
      # 2.2.9   Ensure end-to-end TLS encryption is enabled - TBD
      check_azure_app_service_deployment_slot_value "End-to-End TLS Encryption"                   "${app_name}" "${slot_name}" "${res_group}" "config"                             "web" "Microsoft.Web/sites" "endToEndEncryptionEnabled"         "eq" "true"                          "properties.endToEndEncryptionEnabled"  ""
      # 2.2.10  Ensure 'Remote debugging' is set to 'Off' - TBD
      check_azure_app_service_deployment_slot_value "Remote Debugging"                            "${app_name}" "${slot_name}" "${res_group}" "config"                             "web" "Microsoft.Web/sites" "remoteDebuggingEnabled"            "eq" "false"                         "--remote-debugging-enabled"            ""
      # 2.2.11  Ensure incoming client certificates are enabled and required (if in use) - TBD
      check_azure_app_service_deployment_slot_value "Client Certificates"                         "${app_name}" "${slot_name}" "${res_group}" "config"                             "web" "Microsoft.Web/sites" "clientCertEnabled"                 "eq" "true"                          "clientCertEnabled"                     ""
      # 2.2.12  Ensure managed identities are configured - TBD
      check_azure_app_service_deployment_slot_value "Managed Identities"                          "${app_name}" "${slot_name}" "${res_group}" "config"                             "web" "identity"            "type"                              "eq" "${azure_managed_identity}"     ""                                      ""
      # 2.2.13  Ensure public network access is disabled - TBD
      check_azure_app_service_deployment_slot_value "Public Network Access"                       "${app_name}" "${slot_name}" "${res_group}" "config"                             "web" "Microsoft.Web/sites" "publicNetworkAccess"               "eq" "Disabled"                      "properties.publicNetworkAccess"        ""
      # 2.2.14  Ensure app is integrated with a virtual network - TBD
      check_azure_app_service_deployment_slot_value "Virtual Network Integration"                 "${app_name}" "${slot_name}" "${res_group}" "config"                             "web" "Microsoft.Web/sites" "virtualNetworkSubnetId"            "ne" ""                              ""                                      ""
      # 2.2.15  Ensure configuration is routed through the virtual network integration - TBD
      check_azure_app_service_deployment_slot_value "VNet Image Pull"                             "${app_name}" "${slot_name}" "${res_group}" "config"                             "web" "Microsoft.Web/sites" "vnetRouteAllEnabled"               "eq" "true"                          "properties.vnetRouteAllEnabled"        ""
      check_azure_app_service_deployment_slot_value "VNet Content Share"                          "${app_name}" "${slot_name}" "${res_group}" "config"                             "web" "Microsoft.Web/sites" "vnetContentShareEnabled"           "eq" "true"                          "properties.vnetContentShareEnabled"    ""
      # 2.2.17  Ensure cross-origin resource sharing does not allow all origins - TBD
      check_azure_app_service_deployment_slot_value "Cross-Origin Resource Sharing"               "${app_name}" "${slot_name}" "${res_group}" "config"                             "web" "cors"                "siteConfig.cors.allowedOrigins"    "ne" "*"                             "properties.cors.allowedOrigins"        ""
    done
  done
