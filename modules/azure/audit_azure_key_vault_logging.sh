...
audit_azure_key_vault_logging () {
  ...
  key_vault_ids=$( az keyvault list --query "[].id" --output tsv 2>/dev/null )
  for key_vault_id in ${key_vault_ids}; do
    resource_names=$( az monitor diagnostic-settings list --resource ${key_vault_id} --query "[].name" --output tsv 2>/dev/null )
