...
audit_azure_app_service_apps () {
  ...
  for app_name in ${app_names}; do
    ...
    # 2.1.1   Ensure 'Java version' is currently supported (if in use) - TBD
    check_azure_app_service_app_value "Java Version"                  "${app_name}" "${res_group}" ""                    "javaVersion"                       "eq" "${azure_java_version}"         "--java-version"                                 ""
    check_azure_app_service_app_value "Java Container Version"        "${app_name}" "${res_group}" ""                    "javaContainerVersion"              "eq" "${azure_java_version}"         "--java-container-version"                       ""
    # 2.1.2   Ensure 'Python version' is currently supported (if in use) - TBD
    check_azure_app_service_app_value "Python Version"                "${app_name}" "${res_group}" ""                    "pythonVersion"                     "eq" "${azure_python_version}"       "--python-version"                               ""
    # 2.1.3   Ensure 'PHP version' is currently supported (if in use) - TBD
    check_azure_app_service_app_value "PHP Version"                   "${app_name}" "${res_group}" ""                    "phpVersion"                        "eq" "${azure_php_version}"          "--php-version"                                  ""
    # 2.1.5   Ensure 'FTP State' is set to 'FTPS only' or 'Disabled' - TBD
    check_azure_app_service_app_value "FTP State"                     "${app_name}" "${res_group}" ""                    "ftpState"                          "eq" "${azure_ftp_state}"            "--ftp-state"                                    ""
    # 2.1.6   Ensure 'HTTP version' is set to '2.0' (if in use) - TBD
    check_azure_app_service_app_value "HTTP Version"                  "${app_name}" "${res_group}" ""                    "http20Enabled"                     "eq" "true"                          "--http20-enabled"                               ""
    # 2.1.7   Ensure 'HTTPS Only' is set to 'On' - TBD
    check_azure_app_service_app_value "HTTPS Only"                    "${app_name}" "${res_group}" ""                    "httpsOnly"                         "eq" "true"                          "httpsOnly"                                      ""
    # 2.1.8   Ensure 'Minimum Inbound TLS Version' is set to '1.2' or higher - TBD
    check_azure_app_service_app_value "Minimum Inbound TLS Version"   "${app_name}" "${res_group}" ""                    "minTlsVersion"                     "eq" "1.2"                           "--min-tls-version"                              ""
    # 2.1.9   Ensure end-to-end TLS encryption is enabled - TBD
    check_azure_app_service_app_value "End-to-End TLS Encryption"     "${app_name}" "${res_group}" "Microsoft.Web/sites" "endToEndEncryptionEnabled"         "eq" "true"                          "properties.endToEndEncryptionEnabled"           ""
    # 2.1.10  Ensure 'Remote debugging' is set to 'Off' - TBD
    check_azure_app_service_app_value "Remote Debugging"              "${app_name}" "${res_group}" ""                    "remoteDebuggingEnabled"            "eq" "false"                         "--remote-debugging-enabled"                     ""
    # 2.1.11  Ensure incoming client certificates are enabled and required (if in use) - TBD
    check_azure_app_service_app_value "Client Certificates"           "${app_name}" "${res_group}" "Microsoft.Web/sites" "clientCertEnabled"                 "eq" "true"                          "clientCertEnabled"                              ""
    # 2.1.12  Ensure 'App Service authentication' is set to 'Enabled' - TBD
    check_azure_app_service_app_value "App Service Auth v1"           "${app_name}" "${res_group}" "auth"                "enabled"                           "eq" "true"                          "--enabled"                                      ""
    check_azure_app_service_app_value "App Service Auth v2"           "${app_name}" "${res_group}" "auth"                "platform.enabled"                  "eq" "true"                          "--enabled"                                      ""
    # 2.1.13  Ensure managed identities are configured - TBD
    check_azure_app_service_app_value "Managed Identities"            "${app_name}" "${res_group}" "identity"            "type"                              "eq" "${azure_managed_identity}"     ""                                               ""
    # 2.1.14  Ensure public network access is disabled - TBD
    check_azure_app_service_app_value "Public Network Access"         "${app_name}" "${res_group}" ""                    "publicNetworkAccess"               "eq" "Disabled"                      "properties.publicNetworkAccess"                 ""
    # 2.1.17  Ensure private endpoints used to access App Service apps use private DNS zones - TBD
    check_azure_app_service_app_value "Private DNS Zones"             "${app_name}" "${res_group}" "Microsoft.Web/sites" "dnsConfiguration.dnsServers"       "ne" ""                              ""                                               ""
    # 2.1.18  Ensure app is integrated with a virtual network - TBD
    check_azure_app_service_app_value "Virtual Network Integration"   "${app_name}" "${res_group}" ""                    "virtualNetworkSubnetId"            "ne" ""                              ""                                               ""
    # 2.1.19  Ensure configuration is routed through the virtual network integration - TBD
    # 2.1.20  Ensure all traffic is routed through the virtual network - TBD
    check_azure_app_service_app_value "VNet Image Pull"               "${app_name}" "${res_group}" "Microsoft.Web/sites" "vnetRouteAllEnabled"               "eq" "true"                          "properties.vnetRouteAllEnabled"                 ""
    check_azure_app_service_app_value "VNet Content Share"            "${app_name}" "${res_group}" "Microsoft.Web/sites" "vnetContentShareEnabled"           "eq" "true"                          "properties.vnetContentShareEnabled"             ""
    # 2.1.21  Ensure cross-origin resource sharing does not allow all origins - TBD
    check_azure_app_service_app_value "Cross-Origin Resource Sharing" "${app_name}" "${res_group}" "cors"                "siteConfig.cors.allowedOrigins"    "ne" "*"                             "properties.cors.allowedOrigins"                 ""
    # 2.1.15  Ensure App Service plan SKU supports private endpoints - TBD
