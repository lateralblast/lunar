...
audit_azure_storage_accounts () {
  ...
  for storage_account in ${storage_accounts}; do
    ...
    # 9.3.1.2  Ensure that Storage Account access keys are periodically regenerated
    # 17.1.3   Ensure that Storage Account Access Keys are Periodically Regenerated
    check_azure_storage_account_keys_rotation                                                     "${storage_account}"  "${res_id}"    "${key_rotation}"                                  
    # 9.3.1.1  Ensure that 'Enable key rotation reminders' is enabled for each Storage Account
    # 17.1.1   Ensure that 'Enable key rotation reminders' is enabled for each Storage Account
    check_azure_storage_account_value         "Enable key rotation reminders"                     "${storage_account}"  "${res_group}" "keyPolicy.keyExpirationPeriodInDays" "eq" "${key_rotation}" "--key-expiration-period-in-days"
    # 9.3.1.3  Ensure 'Allow storage account key access' for Azure Storage Accounts is 'Disabled'
    # 17.1.5   Ensure 'Allow storage account key access' for Azure Storage Accounts is 'Disabled'
    check_azure_storage_account_value         "Allow storage account key access"                  "${storage_account}"  "${res_group}" "allowSharedKeyAccess"                "eq" "false"           "--allow-shared-key-access"
    # 9.3.2.1  Ensure Private Endpoints are used to access Storage Accounts
    # 17.2.1   Ensure Private Endpoints are used to access Storage Accounts
    check_azure_storage_account_value         "Private Endpoints are used to access"              "${storage_account}"  "${res_group}" "privateEndpointConnections[0].id"    "ne" ""                ""
    # 9.3.2.2  Ensure that 'Public Network Access' is 'Disabled' for storage accounts
    # 17.2.2   Ensure that 'Public Network Access' is 'Disabled' for storage accounts
    # 2.2.1.1  Ensure public network access is Disabled
    check_azure_storage_account_value         "Public Network Access"                             "${storage_account}"  "${res_group}" "publicNetworkAccess"                         "eq" "Disabled"        "--public-network-access"
    # 9.3.2.3  Ensure default network access rule for storage accounts is set to deny
    # 17.2.3   Ensure default network access rule for storage accounts is set to deny
    check_azure_storage_account_value         "Default network access rule"                       "${storage_account}"  "${res_group}" "networkRuleSet.defaultAction"                "eq" "Deny"            "--default-action"
    # 9.3.3.1  Ensure that 'Default to Microsoft Entra authorization in the Azure portal' is set to 'Enabled' 
    check_azure_storage_account_value         "Microsoft Entra authorization"                     "${storage_account}"  "${res_group}" "defaultToOAuthAuthentication"                "eq" "true"            "defaultToOAuthAuthentication"
    # 9.3.4    Ensure that 'Secure transfer required' is set to 'Enabled'
    check_azure_storage_account_value         "Secure transfer required"                          "${storage_account}"  "${res_group}" "enableHttpsTrafficOnly"                      "eq" "true"            "--https-only"
    # 9.3.5    Ensure 'Allow Azure services on the trusted services list to access this storage account' is Enabled for Storage Account Access
    # 17.6     Ensure 'Allow Azure services on the trusted services list to access this storage account' is Enabled for Storage Account Access
    check_azure_storage_account_value         "Azure services on the trusted services list"       "${storage_account}"  "${res_group}" "networkRuleSet.bypass"                       "eq" "AzureServices"   "--bypass"
    # 9.3.6    Ensure the 'Minimum TLS version' for storage accounts is set to 'Version 1.2'
    # 17.11    Ensure the 'Minimum TLS version' for storage accounts is set to 'Version 1.2'
    check_azure_storage_account_value         "Minimum TLS version"                               "${storage_account}"  "${res_group}" "minimumTlsVersion"                           "eq" "TLS1_2"          "--minimum-tls-version"
    # 9.3.7    Ensure 'Cross Tenant Replication' is not enabled
    # 17.12    Ensure 'Cross Tenant Replication' is not enabled
    check_azure_storage_account_value         "Cross Tenant Replication"                          "${storage_account}"  "${res_group}" "allowCrossTenantReplication"                 "eq" "false"           "--allow-cross-tenant-replication"
    # 9.3.8    Ensure 'Allow blob public access' is set to 'Disabled'
    # 17.13    Ensure that 'Allow Blob Anonymous Access' is set to 'Disabled'
    check_azure_storage_account_value         "Allow Blob Public Access"                          "${storage_account}"  "${res_group}" "allowBlobPublicAccess"                       "eq" "false"           "allowBlobPublicAccess"
    # 9.3.9    Ensure Azure Resource Manager Delete locks are applied to Azure Storage Accounts
    check_azure_resource_manager_lock         "Azure Resource Manager Delete locks are applied"   "${storage_account}"  "${res_group}" "[].level"                                    "eq" "CanNotDelete"    "Microsoft.Storage/storageAccounts"
    # 9.3.10   Ensure Azure Resource Manager ReadOnly locks are applied to Azure Storage Accounts
    check_azure_resource_manager_lock         "Azure Resource Manager ReadOnly locks are applied" "${storage_account}"  "${res_group}" "[].level"                                    "eq" "ReadOnly"        "Microsoft.Storage/storageAccounts"
    # 9.3.11   Ensure Redundancy is set to 'geo-redundant storage (GRS)' on critical Azure Storage Accounts
    # 17.16    Ensure Redundancy is set to 'geo-redundant storage (GRS)' on critical Azure Storage Accounts
    check_azure_storage_account_value         "Redundancy is set to geo-redundant storage (GRS)"  "${storage_account}"  "${res_group}" "sku.name"                                    "eq" "Standard_GRS"    "--sku"
    # 2.1.1.1  Ensure 'Allowed Protocols' for shared access signature (SAS) tokens is set to 'HTTPS Only'
    # 17.1.2   Ensure 'Allowed Protocols' for shared access signature (SAS) tokens is set to 'HTTPS Only'
    check_azure_storage_account_value         "Allow Shared Key Access"                           "${storage_account}"  "${res_group}" "allowSharedKeyAccess"                        "eq" "false"           ""
    # 17.4     Ensure that 'Secure transfer required' is set to 'Enabled'
    check_azure_storage_account_value         "Allowed Protocols for SAS tokens is HTTPS Only"    "${storage_account}"  "${res_group}" "enableHttpsTrafficOnly"                      "eq" "true"            ""
    # 2.1.1.3  Ensure stored access policies (SAP) are used when generating shared access signature (SAS) tokens - Need verification
    check_azure_storage_account_value         "SAP are used when generating SAS tokens"           "${storage_account}"  "${res_group}" "sasPolicy"                                   "ne" ""                ""
    # 2.2.1.2  Ensure Network Access Rules are set to Deny-by-default
    check_azure_storage_account_value         "Network Access Rules are set to Deny-by-default"   "${storage_account}"  "${res_group}" "networkRuleSet.defaultAction"                "eq" "Deny"            " --default-action"
    # 17.5     Ensure that ‘Enable Infrastructure Encryption’ for Each Storage Account in Azure Storage is Set to ‘enabled’
    check_azure_storage_account_value         "Enable Infrastructure Encryption"                  "${storage_account}"  "${res_group}" "encryption.infrastructureEncryption.enabled" "eq" "true"            "infrastructureEncryptionEnabled"
  done
}
