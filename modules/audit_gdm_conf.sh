...
audit_gdm_conf () {
  if [ "$os_name" = "Linux" ]; then
    ...
    if [ -e "$check_file" ]; then
      ...
      check_file_value $check_file AllowRoot eq false hash
      check_file_value $check_file AllowRemoteRoot eq false hash
      check_file_value $check_file Use24Clock eq true hash
