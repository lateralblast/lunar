...
# Refer to Section(s) 8.2 Page(s) 174-5 CIS Red Hat Linux 5 Benchmark v2.1.0
# Refer to Section(s) 8.3 Page(s) 154-5 CIS Red Hat Linux 6 Benchmark v1.2.0
# Refer to Section(s) 11.3 Page(s) 143-4 SLES 11 Benchmark v1.0.0
    ...
    if [ "$os_name" = "Linux" ]; then
      ...
      if [ "$audit_mode" != 2 ]; then
        if [ "$actual_value" != "$warning_message" ]; then
          ...
          if [ "$audit_mode" = 1 ]; then
            score=`expr $score + 1`
            echo "Secure:    Warning banner is set to \"$warning_message\" [$score]"
          fi
        fi
      else
